...
def active_neighbour_count(dimension, x, y, z):
	active = 0
	for x_o, y_o, z_o in itertools.product([-1,0,1], repeat=3):
		if x_o == y_o == z_o == 0:
			continue

		if dimension[(x+x_o, y+y_o, z+z_o)]:
			active += 1
	...
	for x in range(min_x-1, max_x+2):
		for y in range(min_y-1, max_y+2):
			for z in range(min_z-1, max_z+2):
				active_neighbours = active_neighbour_count(dimension, x, y, z)
				if dimension[(x, y, z)]:
					if active_neighbours in [2,3]:
						new_dimension[(x, y, z)] = True
...
for x, line in enumerate(args.input_file):
	for y, cell in enumerate(line.strip()):
		if cell == "#":
			dimension[(x, y, 0)] = True
		else:
			dimension[(x, y, 0)] = False
