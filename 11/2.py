...
def is_occupied(row_idx, row_offset, col_idx, col_offset, area):
	while True:
		row_idx += row_offset
		col_idx += col_offset
		...
		if col_idx < 0 or col_idx >= len(row):
			return 0
		if row[col_idx] == Position.OCCUPIED_SEAT:
			return 1
		if row[col_idx] == Position.EMPTY_SEAT:
			return 0
	return 0
	...
	for row_idx, row in enumerate(area):
		for col_idx, seat in enumerate(row):
			...
			if seat == Position.EMPTY_SEAT and occ_adj == 0:
				...
				changed = True
			elif seat == Position.OCCUPIED_SEAT and occ_adj >= 5:
